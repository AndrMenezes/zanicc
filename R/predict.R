.predictMLNBART <- function(object, newdata, ndpost, output_dir, load, verbose) {
  ...
  object$cpp_obj$Predict(
    newdata, as.integer(ndpost), as.integer(object$ntrees),
    object$forests_dir, output_dir, as.integer(verbose)
  )
  # load results
  if (load) {
    pred <- load_bin_predictions(
      fname = file.path(output_dir, "theta_ij.bin"),
...
.predictZANIBART <- function(object, newdata, type, ndpost, output_dir, load,
  ...
  # Perform the predictions
  switch(type,
    "theta" = object$cpp_obj$ComputePredictProb(
      newdata, as.integer(ndpost),
      as.integer(object$ntrees_theta),
      object$forests_dir, output_dir,
      as.integer(verbose)
    ),
    "zeta" = object$cpp_obj$ComputePredictProbZero(
      newdata, as.integer(ndpost),
  ...
  if (load) {
    pred <- load_bin_predictions(
      fname = file.path(output_dir, paste0(type, "_ij.bin")),
      n = n, d = object$d, m = ndpost
    )
    return(pred)
  }
...
predict.MultinomialBART <- function(object, newdata, ndpost = object$ndpost,
                                    ...
                                    verbose = TRUE, ...) {
  .predictMLNBART(
    object = object, newdata = newdata, ndpost = ndpost,
...
predict.ZANIMBART <- function(object, newdata, type = c("theta", "zeta"),
  ...
  type <- match.arg(type)
  .predictZANIBART(object,
    newdata = newdata, type = type, ndpost = ndpost,
    output_dir = output_dir, load = load, verbose = verbose
  )
}
#' @export
...
predict.DMRegression <- function(object, newdata, type = c("alpha", "theta"),
  ...
  if (type == "theta") {
    return(sweep(predictions, MARGIN = c(1, 3), apply(predictions, c(1, 3), sum), "/"))
  }
  return(predictions)
}
...
predict.ZANIDMRegression <- function(object, newdata, type = c("alpha", "zeta", "theta"),
  ...
  if (!is.null(object$draws_betas_alpha) && !is.null(object$draws_betas_alpha)) {
    ...
    if (type == "zeta") {
      ...
      draws <- load_bin_coefficients(
        fname = ff, p = object$p_zeta, d = object$d,
        m = ndpost
      )
      ...
      draws <- load_bin_coefficients(
        fname = ff, p = object$p_alpha, d = object$d,
        m = ndpost
      )
      res <- apply(draws, 3, function(b) newdata %*% b, simplify = "array")
    }
