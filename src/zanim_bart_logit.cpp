...
ZANIMBARTLogit::ZANIMBARTLogit(const arma::umat &Y, const arma::mat &X_theta,
...
ZANIMBARTLogit::~ZANIMBARTLogit(){};

void ZANIMBARTLogit::SetMCMC(double v0_theta, double v0_zeta,
  ...
  for (int j = 0; j < d; j++) {
    arma::uvec z = Y.col(j) == 0;
    SharedLogitBART *m = new SharedLogitBART(z, X_zeta);
    m->SetMCMC(v0_zeta, ntrees_zeta_, ndpost, nskip, 1, numcut, power,
  ...
  //thetas = arma::zeros<arma::mat>(n, d);

  // Auxiliary vectors/matrices for the back-fitting in shared logit BART
  rt_zi = arma::ones<arma::mat>(n, d);
  fit_0_h = arma::zeros<arma::vec>(n);
  fit_1_h = arma::zeros<arma::vec>(n);
  f0_mu = arma::zeros<arma::mat>(n, d);
  f1_mu = arma::zeros<arma::mat>(n, d);
  f1_lambda = arma::zeros<arma::mat>(n, d);
...
void ZANIMBARTLogit::BackFitZI(int &j, int &t) {
  // Back-fit
  fit_0_h = f0_mu.col(j) - list_bart_zi[j]->g0_trees.col(t);
  fit_1_h = f1_mu.col(j) - list_bart_zi[j]->g1_trees.col(t);
  list_bart_zi[j]->fit_0_h_phi = exp(fit_0_h) % list_bart_zi[j]->phi;
  list_bart_zi[j]->fit_1_h_phi = exp(fit_1_h) % list_bart_zi[j]->phi;
  // Update tree
  ...
  // Re-fit
  f0_mu.col(j) = fit_0_h + list_bart_zi[j]->g0_trees.col(t);
  f1_mu.col(j) = fit_1_h + list_bart_zi[j]->g1_trees.col(t);
}

void ZANIMBARTLogit::UpdateLatentVariables() {
  // \theta_{ij}
  f_lambda = exp(f_mu);
  // thetas = f_lambda.each_col() / arma::sum(f_lambda, 1);
  // zeta_{ij}
  f1_lambda = exp(f1_mu);
  rt_zi = exp(f0_mu) + f1_lambda;
  // zetas = f1_lambda / rt_zi;
  ...
  for (int j=0; j < d; j++) {
    ...
    // Update \gamma_{ij}
    for (int k=0; k < n; k++) list_bart_zi[j]->phi[k] = R::exp_rand() / rt_zi(k, j);
  }
...
void ZANIMBARTLogit::RunMCMC() {
  ...
  for (int i = 0; i < nskip; i++) {
    ...
    // Update trees and their leaf nodes parameters
    for (int j = 0; j < d; j++) {
  ...
  for (int i = 0; i < ndpost; i++) {
    ...
    for (int j = 0; j < d; j++) {
      ...
      if (alpha_random_mult) {
        ...
        for (int k = 0; k < p_theta; k++)
          slp += std::log(list_splitprobs_mult[j][k]);

        alpha_sparse_mult[j] = UpdateAlphaDirchlet(alphas_grid_mult,
    ...
    if (keep_draws) {
      ...
      draws_zeta.slice(i) = f1_lambda / rt_zi;
...
// Predict functions
arma::vec ZANIMBARTLogit::GetMu(Node *tree, const arma::rowvec &x) {
  if (tree->is_leaf) return tree->mu;
  if (x[tree->predictor] <= tree->cutoff) return GetMu(tree->left, x);
  else return GetMu(tree->right, x);
}

void ZANIMBARTLogit::ComputePredictProb(arma::mat &X_, int n_samples,
...
void ZANIMBARTLogit::ComputePredictProbZero(arma::mat &X_, int n_samples,
                                            int ntrees, std::string path_out,
                                            std::string path, int verbose) {
  int np = 2;
  ...
  for (int t = 0; t < n_samples; t++) {
    ...
    for (int j = 0; j < d; j++) {
      ...
      f1 = exp(f1);
      f1 = f1.each_col() / (f1 + exp(f0));
      // Save the predictions of sample h-th and category j
      fout.write(reinterpret_cast<const char*>(f1.memptr()), sizeof(double)*n_);
    }
  }
  fout.close();
  for (int j=0; j<d; j++) files[j].close();
...
// trees
void ZANIMBARTLogit::ComputeVarCount(Node *tree, arma::uvec &varcount) {
  if (tree->is_leaf == 0) {
...
arma::ucube ZANIMBARTLogit::GetVarCount(int n_samples, int ntrees,
...
std::vector<double> ZANIMBARTLogit::LogPredictiveDensity(std::vector<int> &y,
                                                               arma::rowvec &x,
                                                               int n_samples,
                                                               int ntrees_theta,
                                                               int ntrees_zeta,
...
double ZANIMBARTLogit::LogAvgPredictiveDensity(std::vector<int> &y,
...
arma::rowvec ZANIMBARTLogit::InversePosteriorESS(std::vector<int> &y, arma::rowvec &x_cur,
                                                       double mean_prior, double sd_prior,
                                                       int n_samples, int ntrees_theta,
                                                       int ntrees_zeta, std::string path) {

  // Draw from the prior
  double nu = mean_prior + sd_prior * R::norm_rand();
  // Set a log-likelihood threshold
  double u_s = log(R::unif_rand());
  u_s += LogAvgPredictiveDensity(y, x_cur, n_samples, ntrees_theta, ntrees_zeta,
                                 path);
  // Draw an angle
  ...
  arma::rowvec x_star = (x_cur - mean_prior) * cos(theta) + (nu - mean_prior) * sin(theta) + mean_prior;
  // Start
  do {
    if (LogAvgPredictiveDensity(y, x_star, n_samples, ntrees_theta, ntrees_zeta, path) > u_s) break;
    if (theta < 0) theta_min = theta;
    else theta_max = theta;
    theta = theta_min + (theta_max - theta_min) * R::unif_rand();
    x_star = (x_cur - mean_prior) * cos(theta) + (nu - mean_prior) * sin(theta) + mean_prior;
  } while (true);

  return x_star;
}

std::vector<double> ZANIMBARTLogit::GetNormaliseProbsIS(
...
arma::mat ZANIMBARTLogit::SampleInversePosterior(
    std::vector<int> &y, arma::rowvec &x_init,
    double mean_prior, double sd_prior,
    int ndpost, int nskip, int printevery,
    int n_samples, int ntrees_theta,
    int ntrees_zeta, std::string path) {
  // std::vector<double> x_samples(ndpost, 0.0);
  arma::mat x_samples = arma::zeros<arma::mat>(x_init.size(), ndpost);
  arma::rowvec x_cur = x_init;

  // Start MCMC with burn-in iterations.
  double progress = 0;
  for (int i=0; i < nskip; i++) {
    progress = (double) 100 * i / nskip;
    Rprintf("\r");
  ...
  for (int i=0; i < ndpost; i++) {
    ...
    x_cur = InversePosteriorESS(y, x_cur, mean_prior, sd_prior,
                                n_samples, ntrees_theta, ntrees_zeta, path);
    x_samples.col(i) = x_cur;
  }

  return x_samples;
}

void ZANIMBARTLogit::GetMCMCPrediction(arma::rowvec &x,
...
void ZANIMBARTLogit::GetMCMCPredictionLoaded(arma::rowvec &x,
                       std::vector<double> &theta, std::vector<double> &zeta,
...
std::vector<double> ZANIMBARTLogit::LogPredictiveDensitySeq(std::vector<int> &y,
                                                arma::mat X,
                                                int n_samples,
                                                int ntrees_theta,
                                                int ntrees_zeta,
...
arma::vec ZANIMBARTLogit::SampleInversePosteriorSeq(std::vector<int> &y,
                                                          arma::rowvec x_cur,
                                                          double mean_prior,
                                                          double sd_prior,
                                                          int n_rep,
...
RCPP_MODULE(zanim_bart_logit) {

  // Expose class MultinomialBART as "MultinomialBART" on the R side
  Rcpp::class_<ZANIMBARTLogit>("ZANIMBARTLogit")

  // Exposing constructor
  .constructor<arma::umat, arma::mat, arma::mat>()

  // Exposing member functions
  .method("SetMCMC", &ZANIMBARTLogit::SetMCMC)
  .method("RunMCMC", &ZANIMBARTLogit::RunMCMC)
  .method("ComputePredictProb", &ZANIMBARTLogit::ComputePredictProb)
  .method("ComputePredictProbZero", &ZANIMBARTLogit::ComputePredictProbZero)
  .method("GetVarCount", &ZANIMBARTLogit::GetVarCount)
  .method("LogPredictiveDensity", &ZANIMBARTLogit::LogPredictiveDensity)
  .method("LogAvgPredictiveDensity", &ZANIMBARTLogit::LogAvgPredictiveDensity)
  .method("InversePosteriorESS", &ZANIMBARTLogit::InversePosteriorESS)
  .method("SampleInversePosterior", &ZANIMBARTLogit::SampleInversePosterior)
  .method("GetNormaliseProbsIS", &ZANIMBARTLogit::GetNormaliseProbsIS)
  .method("LogPredictiveDensitySeq", &ZANIMBARTLogit::LogPredictiveDensitySeq)
  .method("SampleInversePosteriorSeq", &ZANIMBARTLogit::SampleInversePosteriorSeq)

  // Exposing some attributes
  .field("draws_theta", &ZANIMBARTLogit::draws_theta)
  .field("draws_zeta", &ZANIMBARTLogit::draws_zeta)
  .field("draws_phi", &ZANIMBARTLogit::draws_phi)
  .field("draws_vartheta", &ZANIMBARTLogit::draws_vartheta)

  .field("varcount_mcmc_theta", &ZANIMBARTLogit::varcount_mcmc_theta)
  .field("varcount_mcmc_zeta", &ZANIMBARTLogit::varcount_mcmc_zeta)
  .field("alpha_sparse_mult", &ZANIMBARTLogit::alpha_sparse_mult)
  .field("alpha_sparse_zi", &ZANIMBARTLogit::alpha_sparse_zi)
  .field("sigma_mult_mcmc", &ZANIMBARTLogit::sigma_mult_mcmc)
  .field("avg_leaves_theta", &ZANIMBARTLogit::avg_leaves_theta)
  .field("avg_leaves_zeta", &ZANIMBARTLogit::avg_leaves_zeta)

  // Acceptance rate
  .field("accept_rate_theta", &ZANIMBARTLogit::accept_rate_theta)
  .field("accept_rate_zeta", &ZANIMBARTLogit::accept_rate_zeta)
