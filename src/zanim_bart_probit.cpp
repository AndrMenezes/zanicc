...
ZANIMBARTProbit::ZANIMBARTProbit(int d, int p_theta, int p_zeta)
...
void ZANIMBARTProbit::UpdateLatentVariables() {
  ...
  for (int j=0; j < d; j++) {
    ...
    //     zeta_ij = R::pnorm5(f0_mu(i, j), 0.0, 1.0, 1.0, 0.0);
    //     // Update z_j | \phi_i
    //     p_ij = (1.0 - zeta_ij) * exp(-f_lambda(i, j) * bart_mult->phi(i));
    //     p_ij /= (zeta_ij + p_ij);
    //     prob_0(i, j) = p_ij;
    //   }
    // }
