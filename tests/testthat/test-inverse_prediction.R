...
test_that("inverse prediction with bspline", {
  ...
  mod$SetupMCMC(
    v0_theta = 1.5 / sqrt(2), ntrees_theta = 25L, ntrees_zeta = 25L,
  ...
  mod$cpp_obj$LogPredictiveDensity(
    y_cur, x_cur, n_samples, ntrees_theta, ntrees_zeta,
    path_res
  )

  mean_prior <- mean(X)
  sd_prior <- sd(X)
  mod$cpp_obj$InversePosteriorESS(
    y_cur, x_cur, mean_prior, sd_prior, n_samples,
  ...
  ini <- proc.time()
  probs <- mod$cpp_obj$GetNormaliseProbsIS(
    y_cur, matrix(X_prior, ncol = 1),
    ndpost + nskip,
  ...
  ini <- proc.time()
  probs1 <- mod$cpp_obj$GetNormaliseProbsIS(
    y_cur, matrix(X_prior1, ncol = 1),
    ndpost + nskip,
    n_samples, ntrees_theta, ntrees_zeta,
    path_res
  )
  ...
  out <- mod$cpp_obj$SampleInversePosterior(
    y_cur, x_cur, mean_prior, sd_prior,
  ...
  list_out <- parallel::mclapply(X = seq_len(n_test), FUN = function(i) {
    mod$cpp_obj$SampleInversePosterior(
      y_cur, x_true[i], mean_prior, sd_prior,
      ndpost, nskip, printevery, n_samples,
      ntrees_theta, ntrees_zeta, path_res
    )
  }, mc.cores = 4L)
  ...
  for (i in seq_len(n_test - 1)) {
    ...
    hist(list_out[[i]], xlim = xlim)
    abline(v = x_true[i], col = "red")
  }
  ...
  ntrees_theta <- 25L
  ntrees_zeta <- 25L
  x_post <- mod$cpp_obj$SampleInversePosteriorSeq(
    Y[2L, ], X[2L, ], mean(X), sd(X),
