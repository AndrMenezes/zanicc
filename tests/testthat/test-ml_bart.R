...
test_that("bspline multinomial", {
  ...
  d <- 4L
  set.seed(1212)
  sim_tmp <- sim_data_multinomial_bspline_curve(
    d = 4, n = n_sample,
    n_trials = n_trials
  )
  Y <- sim_tmp$Y
  ...
  mod_nonshared$SetupMCMC(
    v0 = 3.5 / sqrt(2), ntrees = 200L, ndpost = 1000L,
...
test_that("test predict and ppd functions for multinomialBART class", {
  ...
  ml_bart <- zanicc(
    Y = Y_train, X_count = X_train, model = "ml_bart",
    ntrees_theta = 20L, ndpost = 1000L, nskip = 3000L,
    path = path_res, keep_draws = TRUE, save_trees = TRUE
  )

  pred1 <- predict.MultinomialBART(ml_bart,
    newdata = X_train, load = TRUE,
    output_dir = path_res
  )
  expect_equal(pred1, ml_bart$draws_theta)

  ppd1 <- .ppd_multinomial_batch(
    n_trials = rowSums(Y_train), output_dir = path_res,
    n_pred = nrow(Y_train), d = d, ndpost = 1000,
    batch_size = 50L, relative = TRUE,
    printevery = 100
  )
  ppd2 <- ppd(
    object = ml_bart, in_sample = FALSE, output_dir = path_res,
  ...
  # Out of sample
  pred2 <- predict.MultinomialBART(ml_bart,
    newdata = X_test, load = TRUE,
    output_dir = path_res
  )
  par(mfrow = c(2, 2))
  ...
  res2 <- ppd(
    object = ml_bart, in_sample = FALSE, draws_prob = pred2,
...
test_that("friedman trinomial", {
  ...
  mbart$SetupMCMC(
    ntrees = 100L, ndpost = 5000, nskip = 2000,
  ...
  data_dart <- data.frame(
    prob = c(prob_vc_d), covariate = rep(1:ncol(X), times = 3),
    category = rep(1:3, each = ncol(X)),
    split_prior = "dirichlet"
  )
  data_bart <- data.frame(
    prob = c(prob_vc), covariate = rep(1:ncol(X), times = 3),
    category = rep(1:3, each = ncol(X)),
    split_prior = "uniform"
  )
