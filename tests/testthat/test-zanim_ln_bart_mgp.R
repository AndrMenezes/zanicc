test_that("Check if the ZANIM-LN-BART with MGP prior is working", {
  ...
  for (i in seq_len(9)) {
    ...
    list_plots_mgp[[i]] <- pheatmap::pheatmap(
      mat = abs(mat_mgp),
      main = paste0("Iteration: ", indices[i]),
      cluster_rows = FALSE, cluster_cols = FALSE
      # , color = colorRampPalette(c("blue", "white", "red"))(100)
    )$gtable
    list_plots_q10[[i]] <- pheatmap::pheatmap(
      mat = abs(mat_q10), main = paste0("Iteration: ", indices[i]),
      cluster_rows = FALSE, cluster_cols = FALSE
    )$gtable
  }
  ...
  p_zanimlnbart_q10 <- ggplot(data = data_sim) +
    ...
    facet_wrap(~category, scales = "free_y") +
    geom_line(
      data = df_summaries_theta, mapping = aes(x = x, y = median),
      col = "dodgerblue"
    ) +
    geom_ribbon(
      data = df_summaries_theta, aes(x = x, ymin = ci_lower, ymax = ci_upper),
...
test_that("Estimate theta using the ZANIM-LN-BART", {
  ...
  # devtools::load_all()
  m <- zanicc(Y,
    X_count = X, X_zi = X, ntrees_theta = 10, ndpost = 10, nskip = 100, model = "zanim_ln_bart",
  ...
  frob_vartheta <- compute_frob_chain(
  ...
  ggplot(data = data_sim) +
    ...
    facet_wrap(~category, scales = "free_y") +
    geom_line(
      data = df_summaries_zeta, mapping = aes(x = x, y = median),
      col = "dodgerblue"
    ) +
    geom_ribbon(
      data = df_summaries_zeta, aes(x = x, ymin = ci_lower, ymax = ci_upper),
      fill = "dodgerblue", alpha = 0.3
    )
  ...
  p_mlnbart <- ggplot(data = data_sim) +
    ...
    facet_wrap(~category, scales = "free_y") +
    geom_line(
      data = df_summaries_theta3, mapping = aes(x = x, y = median),
      col = "dodgerblue"
    ) +
    geom_ribbon(
      data = df_summaries_theta3, aes(x = x, ymin = ci_lower, ymax = ci_upper),
      fill = "dodgerblue", alpha = 0.3
    )
