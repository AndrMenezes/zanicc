...
test_that("bspline probit", {
  ...
  n_sample <- 400L
  d <- 4L
  tmp <- sim_data_zanim_bspline_curve(
  ...
  mod$SetupMCMC(
    v0_theta = 3.5 / sqrt(2), ntrees_theta = 20L, ntrees_zeta = 20L,
    ndpost = 2000L, nskip = 200L, update_sigma_theta = TRUE,
    path = path_res, keep_draws = TRUE
  ) # tempdir())
  mod$RunMCMC()
  mod$elapsed_time
...
test_that("ZANIM-BART with sampling zeros", {
  ...
  table(rowSums(Y) == 0)
  cbind(
    all_zeros = colMeans(Y == 0), structural_zeros = colMeans(1 - Z),
  ...
  p_zeta <- ggplot(data = data_sim) +
    ...
    geom_rug(
      data = dplyr::filter(data_sim, total == 0L, z == 0),
      mapping = aes(y = NA_real_, x = x), col = "red"
    ) +
    geom_rug(
      data = dplyr::filter(data_sim, total == 0L, z == 1),
      mapping = aes(y = NA_real_, x = x), col = "black"
    ) +
    geom_line(data = data_zeta, mapping = aes(x = x, y = median, col = model)) +
    geom_ribbon(data = data_zeta, aes(
      x = x, ymin = ci_lower, ymax = ci_upper,
      fill = model
    ), alpha = 0.3) +
    labs(
      y = latex2exp::TeX(r'(Zero-inflation probabilities, $\zeta_{ij}$)'),
...
test_that("test predict and ppd methods", {
  ...
  # Predict function
  pred_theta <- predict.ZANIMBART(
    object = zanim_bart, newdata = X_train, type = "theta",
    load = TRUE, output_dir = path_res
  )
  expect_equal(pred_theta, zanim_bart$draws_theta)
  pred_zeta <- predict.ZANIMBART(
    object = zanim_bart, newdata = X_train, type = "zeta",
    load = TRUE, output_dir = path_res
  )
  ...
  ppd2 <- ppd(
    object = zanim_bart, in_sample = FALSE, draws_prob = pred_theta,
    draws_zeta = pred_zeta, n_trials = n_trials_train
  )
  ppd3 <- ppd(
    object = zanim_bart, in_sample = FALSE, n_trials = n_trials_train,
  ...
  pred2_theta <- predict.ZANIMBART(
    object = zanim_bart, newdata = X_test,
    type = "theta", load = TRUE,
    output_dir = path_res
  )
  rowSums(pred2_theta[, , 1])
  pred2_zeta <- predict.ZANIMBART(
    object = zanim_bart, newdata = X_test,
    type = "zeta", load = TRUE,
    output_dir = path_res
  )
  ...
  ppd2 <- ppd(
    object = zanim_bart, in_sample = FALSE, draws_prob = pred2_theta,
    draws_zeta = pred2_zeta, n_trials = n_trials_test
  )
  ppd3 <- ppd(
    object = zanim_bart, in_sample = FALSE, n_trials = n_trials_test,
  ...
  # Counts
  ppd2 <- ppd(
    object = zanim_bart, in_sample = FALSE, relative = FALSE,
