...
test_that("bspline", {
  ...
  mod$SetupMCMC(
    v0_theta = 1.5 / sqrt(2), ntrees_theta = 10L, ntrees_zeta = 10L,
  ...
  p_theta <- ggplot(data = data_sim) +
    ...
    geom_line(
      data = data_theta, mapping = aes(x = x, y = median),
      col = "dodgerblue"
    ) +
    geom_ribbon(
      data = data_theta, aes(x = x, ymin = ci_lower, ymax = ci_upper),
      fill = "dodgerblue", alpha = 0.3
    )
  p_theta
  save_plot(
    filename = file.path(path_res, "theta.png"), plot = p_theta,
  ...
  p_zeta <- ggplot(data = data_sim) +
    ...
    geom_line(
      data = data_zeta, mapping = aes(x = x, y = median),
      col = "dodgerblue"
    ) +
    geom_ribbon(
      data = data_zeta, aes(x = x, ymin = ci_lower, ymax = ci_upper),
      fill = "dodgerblue", alpha = 0.3
    ) +
    facet_wrap(~category, scales = "free_y")
  p_zeta
  save_plot(
    filename = file.path(path_res, "zeta.png"), plot = p_zeta,
    bg = "white", base_height = 7.0
  )
  ...
  p <- bayesplot::ppc_ecdf_overlay_grouped(
    y = c(Y), yrep = t(ppc_appended),
    group = g_vec
  )
  save_plot(
    filename = file.path(path_res, "ppc_bspline.png"), plot = p,
    bg = "white", base_height = 7.0
  )
...
test_that("friedman", {
  ...
  n_trials <- 500L
  n_sample <- 500L
  tmp <- sim_data_zanim_friedman(
    n = n_sample, n_trials = n_trials, p_theta = 20L,
    p_zeta = 20L
  )
  X_theta <- tmp$X_theta
  X_zeta <- tmp$X_zeta
  ...
  mod$SetupMCMC(
    v0_theta = 1.5 / sqrt(2), ntrees_theta = 100L, ntrees_zeta = 100L,
  ...
  vc_theta_bart_2 <- mod$cpp_obj$GetVarCount(
    mod$ndpost, mod$ntrees_theta, "theta",
    mod$path
  )
  expect_equal(vc_theta_bart, vc_theta_bart_2)

  vc_zeta_bart <- mod$GetVarCount(parameter = "zeta")
  vc_zeta_bart_2 <- mod$cpp_obj$GetVarCount(
    mod$ndpost, mod$ntrees_zeta, "zeta",
    mod$path
  )
  ...
  path_res2 <- file.path(
    "./tests/testthat/zanim_sharedlogit", time_id, "sparse",
    "draws"
  )
  ...
  vc_theta_dart_2 <- mod2$cpp_obj$GetVarCount(
    mod2$ndpost, mod2$ntrees_theta, "theta",
    mod2$path
  )
  expect_equal(vc_theta_dart, vc_theta_dart_2)

  vc_zeta_dart <- mod2$GetVarCount(parameter = "zeta")
  vc_zeta_dart_2 <- mod2$cpp_obj$GetVarCount(
    mod2$ndpost, mod2$ntrees_zeta, "zeta",
    mod2$path
  )
  ...
  data_bart <- data.frame(
    prob = c(prob_vc_theta_bart),
    covariate = rep(1:ncol(X_theta), times = 3),
    category = rep(1:3, each = ncol(X_theta)),
    split_prior = "uniform"
  )
  data_vc_theta <- rbind(data_bart, data_dart)
  p_vc_theta <- ggplot(data_vc_theta, aes(
    x = covariate, y = prob,
  ...
  save_plot(
    filename = file.path(path_res, "prob_vc_theta.png"), plot = p_vc_theta,
    bg = "white", base_height = 7.0
  )
  ...
  p_vc_zeta <- ggplot(data_vc_zeta, aes(x = covariate, y = prob, col = split_prior)) +
    ...
    labs(x = "Covariate k", y = "Prob[k in model]")
  save_plot(
    filename = file.path(path_res, "prob_vc_zeta.png"), plot = p_vc_zeta,
    bg = "white", base_height = 7.0
  )

  # Workflow for the log-likelihood
  mod$ComputePredictions(
    X = X_theta, n_samples = 2000L, path_out = path_res,
    parameter = "theta"
  )
  mod$ComputePredictions(
    X = X_zeta, n_samples = 2000L, path_out = path_res,
    parameter = "zeta"
  )
  ll_bart <- mod$LogPredictiveLikelihood(
    Y = Y, n_samples = 2000L, path = path_res,
    n_pred = nrow(Y), ncores = 20L,
    logfile = "log.txt"
  )
  # For DART
  mod2$ComputePredictions(
    X = X_theta, n_samples = 2000L, path_out = path_res2,
    parameter = "theta"
  )
  mod2$ComputePredictions(
    X = X_zeta, n_samples = 2000L, path_out = path_res2,
    parameter = "zeta"
  )
  ll_dart <- mod2$LogPredictiveLikelihood(
    Y = Y, n_samples = 2000L, path = path_res2,
    n_pred = nrow(Y), ncores = 20L,
    logfile = "log.txt"
  )
  dim(ll_dart)
