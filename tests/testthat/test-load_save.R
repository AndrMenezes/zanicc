test_that("test save-load model", {
  ...
  pred <- predict(zanim_bart_loaded,
    newdata = head(X_train), type = "theta",
    output_dir = path_res, load = TRUE
  )
  expect_equal(pred, zanim_bart_loaded$draws_theta[1:6, , ])
  predz <- predict(zanim_bart_loaded,
    newdata = head(X_train), type = "zeta",
    output_dir = path_res, load = TRUE
  )
  ...
  pred <- predict(zanim_ln_bart_loaded,
    newdata = head(X_train), type = "theta",
    output_dir = path_res, load = TRUE
  )
  expect_equal(pred, zanim_ln_bart_loaded$draws_theta[1:6, , ])
  predz <- predict(zanim_ln_bart_loaded,
    newdata = head(X_train), type = "zeta",
    output_dir = path_res, load = TRUE
  )
