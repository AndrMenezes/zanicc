test_that("fit models in pollen data", {
  ...
  # ML-BART
  ml_bart <- zanicc(
    Y = Y, X_count = Xscaled, model = "ml_bart",
    ntrees_theta = NTREES, ndpost = NDPOST, nskip = NSKIP
  )
  # MLN-BART
  mln_bart <- zanicc(
    Y = Y, X_count = Xscaled, model = "mln_bart",
    ntrees_theta = NTREES, ndpost = NDPOST, nskip = NSKIP
  )

  # ZANIM-BART
  zanim_bart <- zanicc(
    Y = Y, X_count = Xscaled, X_zi = Xscaled, model = "zanim_bart",
  ...
  zanim_ln_bart <- zanicc(
    Y = Y, X_count = Xscaled, X_zi = Xscaled,
  ...
  dm_reg <- zanicc(
    Y = Y, X_count = Xwint, model = "dm_reg",
    ndpost = 10, nskip = 10, save_draws = TRUE
  )

  # ZANIDM-reg
  zanidm_reg <- zanicc(
    Y = Y, X_count = Xwint, X_zi = Xwint, model = "zanidm_reg",
    ndpost = NDPOST, nskip = NSKIP
  )

  # ZANIM-LN-reg
  zanim_ln_reg <- zanicc(
    Y = Y, X_count = Xwint, X_zi = Xwint,
  ...
  cbind(
    kl_ml_bart, kl_mln_bart, kl_zanim_bart, kl_zanim_ln_bart,
  ...
  df <- data.frame(
    x = c(
      kl_chain_ml_bart, kl_chain_zanim_bart, kl_chain_zanim_ln_bart,
      kl_chain_zanim_ln_reg, kl_chain_dm_reg
    ),
    model = rep(c(
      "ML-BART", "ZANIM-BART", "ZANIM-LN-BART",
      "ZANIM-LN-reg", "DM-reg"
    ), each = NDPOST)
  )
  ...
  df <- data.frame(
    x = c(kl_chain_ml_bart, kl_chain_zanim_bart, kl_chain_dm_reg),
  ...
  compute_metrics <- function(true_values, draws, kl_fun = zanicc::compute_kl_simplex) {
    ...
    c(
      kl = mean(kl_fun(true_values, estimates)),
  ...
  res <- lapply(
    list(y_ml_bart, y_mln_bart, y_zanim_bart, y_zanim_ln_bart, y_zanidm_reg, y_dm_reg),
  ...
  df <- data.frame(
    x = c(
      t_ml_bart, t_mln_bart, t_zanim_bart,
      t_zanim_ln_bart, t_zanidm_reg, t_dm_reg
    ),
    model = rep(c(
      "ML-BART", "MLN-BART", "ZANIM-BART", "ZANIM-LN-BART",
      "ZIDM-reg", "DM-reg"
    ), each = NDPOST)
  )
  df$model <- forcats::fct_relevel(
    df$model,
  ...
  p_gdi <- ggplot(data = df, aes(x = x)) +
    ...
    ggtitle("GDI")
  save_plot(
    filename = file.path(path_res, "gdi_density.png"), plot = p_gdi, base_height = 8,
    bg = "white"
  )
  # correlation
  ...
  p_sum_cor <- ggplot(data = df, aes(x = x)) +
    ...
    ggtitle("sum(cor(Y))")
  save_plot(
    filename = file.path(path_res, "sum_cor_Y_density.png"), plot = p_sum_cor,
  ...
  p_avg_zero <- ggplot(data = df, aes(x = x)) +
    ...
    ggtitle("mean(Y == 0)")
  save_plot(
    filename = file.path(path_res, "mean_Y0_density.png"), plot = p_avg_zero,
    base_height = 8, bg = "white"
  )

  p_grid <- plot_grid(p_avg_zero, p_gdi)
  save_plot(
    filename = file.path(path_res, "zero_gdi.png"), plot = p_grid,
    base_height = 8, bg = "white"
  )
  ...
  for (j in seq_len(d)) {
    ...
    save_plot(
      filename = file.path(path_res, paste0("prop_zero__", taxa_names[j], ".png")),
