test_that("test out of sample predictions in ZANIM-LN-BART", {
  ...
  zanim_ln_bart <- zanicc(
    Y = Y_train, X_count = X_train, X_zi = X_train,
  ...
  # Predict function
  pred_theta <- predict(
    object = zanim_ln_bart, newdata = X_train, type = "theta",
    load = TRUE, output_dir = path_res
  )
  expect_equal(pred_theta, zanim_ln_bart$draws_theta)
  pred_zeta <- predict(
    object = zanim_ln_bart, newdata = X_train, type = "zeta",
    load = TRUE, output_dir = path_res
  )
  ...
  ppd2 <- ppd(
    object = zanim_ln_bart, in_sample = FALSE, draws_prob = pred_theta,
    draws_zeta = pred_zeta, draws_chol_Sigma_V = draws_chol_Sigma_V,
    n_trials = n_trials_train
  )
  ppd3 <- ppd(
    object = zanim_ln_bart, in_sample = FALSE, n_trials = n_trials_train,
  ...
  pred2_theta <- predict(
    object = zanim_ln_bart, newdata = X_test, type = "theta",
    load = TRUE, output_dir = path_res
  )
  rowSums(pred2_theta[, , 1])
  pred2_zeta <- predict(
    object = zanim_ln_bart, newdata = X_test, type = "zeta",
    load = TRUE, output_dir = path_res
  )
  x11()
  par(mfrow = c(2, 2))
  ...
  ppd2 <- ppd(
    object = zanim_ln_bart, in_sample = FALSE, draws_prob = pred2_theta,
    draws_chol_Sigma_V = draws_chol_Sigma_V, draws_zeta = pred2_zeta,
    n_trials = n_trials_test
  )
  ppd3 <- ppd(
    object = zanim_ln_bart, in_sample = FALSE, n_trials = n_trials_test,
  ...
  cowplot::plot_grid(
    ...
    bayesplot::ppc_ecdf_overlay(y = Y_test_rel[, 4L], yrep = ppd3[, , 4L]),
    ncol = 4
  )
  ...
  # Counts
  ppd2 <- ppd(
    object = zanim_ln_bart, in_sample = FALSE, relative = FALSE,
...
test_that("test ZANIM-LN-BART with different covariance structures", {
  ...
  list_sim <- sim_data_zanim_ln_bspline_curve(
  ...
  for (j in seq_len(d)) {
    ...
    #      main = paste0("j = ", j))
    # abline(0, 1, col = "red")
    plot(true_thetas[, j], rowMeans(m_full$cpp_obj$draws_theta[, j, ]),
      xlab = "true", ylab = "estimate",
      main = paste0("j = ", j)
    )
    abline(0, 1, col = "red")
    plot(true_thetas[, j], rowMeans(m_fm$cpp_obj$draws_theta[, j, ]),
      xlab = "true", ylab = "estimate",
      main = paste0("j = ", j)
    )
    abline(0, 1, col = "red")
  ...
  for (i in sample.int(n_sample, 9)) {
    ...
    plot(x_full,
      type = "l", ylab = expression(phi[i]), ylim = ylim,
...
test_that("test out of sample predictions in ZANIM-LN-BART", {
  ...
  # n_samples <- NDPOST
  mod$cpp_obj$ComputePredictProb(
    X_test, NDPOST, mod$ntrees_theta,
    path_res, path_res,
    as.integer(TRUE)
  )
  draws_theta_test <- load_bin_predictions(
    fname = file.path(path_res, "theta_ij.bin"),
  ...
  par(mfrow = c(2, 3))
  for (j in seq_len(d)) {
    plot(true_thetas_train[, j], rowMeans(mod$draws_theta[, j, ]),
      main = j,
  ...
  for (j in seq_len(d)) {
    plot(true_thetas_test[, j], rowMeans(draws_theta_test[, j, ]),
      main = j,
      xlab = "true", ylab = "posterior mean"
    )
    abline(0, 1, col = "red")
  ...
  mod$cpp_obj$ComputePredictProbZero(
    X_test, NDPOST, mod$ntrees_zeta, path_res,
    path_res, as.integer(TRUE)
  )
  draws_zeta_test <- load_bin_predictions(
    fname = file.path(path_res, "zeta_ij.bin"),
    n = nrow(X_test), d = mod$d, m = NDPOST
  )
  ...
  for (k in seq_len(NDPOST)) {
    if (k %% 100 == 0) cat(k, "\n")
    y_ppc_out[k, , ] <- .rzanim_vec(
      n = nrow(Y_test), sizes = rowSums(Y_test),
  ...
  x11()
  cowplot::plot_grid(plotlist = list_plots, ncol = 1)
