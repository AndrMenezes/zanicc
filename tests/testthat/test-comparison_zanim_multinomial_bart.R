...
test_that("friedman", {
  ...
  mod_zanim$SetupMCMC(
    v0_theta = 1.5 / sqrt(2), ntrees_theta = NTREES,
  ...
  mod_zanim$ComputePredictions(
    X = X, n_samples = NDPOST, path_out = path_res_1,
    parameter = "theta"
  )
  mod_zanim$ComputePredictions(
    X = X, n_samples = NDPOST, path_out = path_res_1,
  ...
  ll_zanim <- mod_zanim$LogPredictiveLikelihood(
    Y = Y, n_samples = NDPOST,
  ...
  mod_mult$SetupMCMC(
    v0 = 1.5 / sqrt(2), ntrees = NTREES, ndpost = NDPOST,
  ...
  save_plot(
    filename = file.path(path_res_1, "prob_vc_theta.png"), plot = p_vc_theta,
  ...
  p_vc_zeta <- ggplot(data_dart_zeta, aes(x = covariate, y = prob)) +
    ...
    labs(x = "Covariate k", y = "Prob[k in model]")
  save_plot(
    filename = file.path(path_res_1, "prob_vc_zeta.png"), plot = p_vc_zeta,
    bg = "white", base_height = 7.0
  )
