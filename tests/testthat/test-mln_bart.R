test_that("multinomial logistic normal BART", {
  ...
  devtools::load_all()
  self <- zanicc(
    Y = Y, X_count = X, model = "mln_bart", ndpost = 1000,
...
test_that("predict and ppd functions", {
  ...
  # devtools::load_all()
  mln_bart <- zanicc(
    Y = Y_train, X_count = X_train, model = "mln_bart",
  ...
  # Predict function
  pred <- predict(
    object = mln_bart, newdata = X_train, load = TRUE,
    output_dir = path_res
  )
  ...
  draws_chol_Sigma_V <- load_bin_predictions(
    file.path(path_res, "chol_Sigma_V.bin"), d - 1,
    d - 1, 1000
  )
  ...
  ppd2 <- ppd(
    object = mln_bart, in_sample = FALSE, draws_prob = pred,
    draws_chol_Sigma_V = draws_chol_Sigma_V, n_trials = n_trials_train
  )
  ppd3 <- ppd(
    object = mln_bart, in_sample = FALSE, n_trials = n_trials_train,
  ...
  pred2 <- predict.MultinomialLNBART(
    object = mln_bart, newdata = X_test, load = TRUE,
    output_dir = path_res
  )
  x11()
  par(mfrow = c(2, 2))
  ...
  ppd2 <- ppd(
    object = mln_bart, in_sample = FALSE, draws_prob = pred2,
    draws_chol_Sigma_V = draws_chol_Sigma_V, n_trials = n_trials_test
  )
  ppd3 <- ppd(
    object = mln_bart, in_sample = FALSE, n_trials = n_trials_test,
  ...
  # Counts
  ppd2 <- ppd(
    object = mln_bart, relative = FALSE, in_sample = FALSE,
