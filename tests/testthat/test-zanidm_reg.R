test_that("zanidm linear reg is working", {
  ...
  path_res <- file.path("./tests/testthat/zanidm_reg", time_id, "draws")
  ...
  pred_alpha <- predict.ZANIDMRegression(mod, newdata = X, type = "alpha")
  # expect_equal(pred_alpha, mod$draws_alpha)
  pred_theta <- predict.ZANIDMRegression(mod, newdata = X, type = "theta")
  # expect_equal(pred_theta, mod$draws_theta)
  pred_zeta <- predict.ZANIDMRegression(mod, newdata = X, type = "zeta")
